...
export default function bookinghome() {
  ...
  const USER_REF = ref(FIREBASE_DB, "users/" + USER_SESSION?.uid + "/bookings");
  const [bookings, setBookings] = useState(Array<Bookings>);
  useEffect(() => {
    let tempBookings = Array<Bookings>();
    get(USER_REF)
    .then((snapshots) => {
      if(snapshots.exists()){
        snapshots.forEach((snapshot) => {
          if(!snapshot.val().complete && !snapshot.val().cancelled){
      ...
      <ScrollView contentContainerStyle={styles.bookingCont}>
          {
            ...
            bookings.map((booking) => (
              <Link push href={`/client/booking?id=${booking.book_id}`}  key={booking.book_id}>
